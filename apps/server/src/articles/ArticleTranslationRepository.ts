...
function parseAllowedRoles(value: string | string[] | null): string[] {
  ...
  if (Array.isArray(value)) {
    return value.filter((item): item is string => typeof item === "string" && item !== "admin");
  }

  try {
    const parsed = JSON.parse(value) as unknown;
    return Array.isArray(parsed) ? parsed.filter((item): item is string => typeof item === "string" && item !== "admin") : [];
  } catch {
    return [];
  }
}
