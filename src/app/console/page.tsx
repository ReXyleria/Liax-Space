...
import { DashboardEcharts } from "@/components/console/dashboard-echarts";
...
function formatArticleDate(locale: Locale, date: Date | null | undefined) {
  if (!date) {
    return t(locale, "consoleUnpublished");
  }
...
export default async function ConsoleDashboardPage({
  ...
  if (!canViewAnalytics(user)) {
    redirect(canManageArticles(user) ? "/console/articles" : "/console/account?section=profile");
  }
  ...
  return (
    <div className="space-y-8">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight">{t(locale, "consoleDashboard")}</h1>
          <p className="mt-1 text-sm text-muted-foreground">{t(locale, "consoleDashboardDescription")}</p>
        </div>
        {stats ? (
          <Link
      ...
      {error ? (
        <Card className="flex items-center justify-between gap-4 border-destructive/20 bg-destructive/5 p-5">
          <div>
            <p className="font-medium text-destructive">{t(locale, "consoleDashboardLoadFailed")}</p>
      ...
      {stats ? (
        <>
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-5">
            <StatCard label={t(locale, "consoleArticles")} value={stats.totalArticles} icon={FileText} gradient="bg-blue-500" />
            <StatCard label={t(locale, "consoleUsersLabel")} value={stats.totalUsers} icon={Users} gradient="bg-emerald-500" />
            <StatCard label={t(locale, "consoleCommentsLabel")} value={stats.totalComments} icon={MessageSquare} gradient="bg-violet-500" />
            <StatCard label={t(locale, "consoleGuestbookLabel")} value={stats.totalGuestbook} icon={MessageSquareText} gradient="bg-amber-500" />
            <StatCard label={t(locale, "consoleTodayVisits")} value={stats.todayVisits} icon={Eye} gradient="bg-rose-500" />
          </div>
          ...
          <div className="grid gap-6 lg:grid-cols-[1.1fr_0.9fr]">
            <Card>
              <CardContent className="p-5">
                <div className="mb-3 flex items-center justify-between">
                  <h2 className="font-semibold">{t(locale, "consoleRecentPublished")}</h2>
                  <Link href="/console/articles" className="text-xs text-muted-foreground hover:text-foreground">
                    {t(locale, "consoleViewAll")}
                  </Link>
                </div>
                {stats.recentArticles.length ? (
                  <div className="space-y-1.5">
                    {stats.recentArticles.map((article) => (
                      ...
                      >
                        ...
                        <Link
                          href={`/console/articles/${article.id}/edit`}
                          ...
                          title={t(locale, "consoleEditArticle")}
                        >
                  ...
                  <p className="py-6 text-center text-sm text-muted-foreground">{t(locale, "consoleNoPublishedArticles")}</p>
                )}
              </CardContent>
            </Card>
            ...
            <Card>
              <CardContent className="p-5">
                <h2 className="mb-3 font-semibold">{t(locale, "consolePopularArticles")}</h2>
                  ...
                  <div className="space-y-2">
                    {stats.popularArticles.map((article, index) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <p className="truncate text-sm font-medium">{article.title}</p>
                          <p className="text-xs text-muted-foreground">{article.viewCount} {t(locale, "consoleReadsLabel")}</p>
                        </div>
                      </Link>
                    ))}
                  </div>
                ) : (
                  <p className="py-6 text-center text-sm text-muted-foreground">{t(locale, "consoleNoArticleData")}</p>
                )}
              </CardContent>
            </Card>
          </div>
        ...
        <Card className="border-dashed py-12 text-center">
          ...
          <p className="mt-3 font-medium">{t(locale, "consoleNoDashboardData")}</p>
          <p className="mt-1 text-sm text-muted-foreground">
            {t(locale, "consoleNoDashboardDataDescription")}
          </p>
        </Card>
      )}
    </div>
