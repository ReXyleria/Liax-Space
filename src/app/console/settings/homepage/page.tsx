...
import { HomepageAppearanceForm } from "@/components/console/homepage-appearance-form";
...
export default async function ConsoleHomepageSettingsPage() {
  const [locale, user, { settings, error }] = await Promise.all([
    getConsoleLocale(),
    requireConsoleAccess("/console/settings/homepage"),
    getSettingsMap()
  ]);
  ...
  return (
    <div className="space-y-6">
      <div>
        <p className="text-sm font-medium text-primary">{t(locale, "consoleAppearance")}</p>
        <h1 className="text-3xl font-semibold">{t(locale, "consoleHomeNavigation")}</h1>
