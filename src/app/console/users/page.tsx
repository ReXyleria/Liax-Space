...
import { CreateUserDialog } from "@/components/console/create-user-dialog";
import { UserRowForm } from "@/components/console/user-row-form";
...
export default async function ConsoleUsersPage({
  ...
  const [user, locale] = await Promise.all([
    requireConsolePermission(canManageUsers, "/console/users"),
    getConsoleLocale()
  ]);
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div>
          <h1 className="text-3xl font-semibold">{t(locale, "consoleUserManagement")}</h1>
          <p className="mt-2 text-muted-foreground">{t(locale, "consoleUserDescription")}</p>
        </div>
      ...
      <form className="grid gap-2 md:grid-cols-[1fr_180px_220px_auto]">
        <Input name="q" placeholder={t(locale, "consoleUserSearchPlaceholder")} defaultValue={params.q ?? ""} />
        <Select
          name="status"
          defaultValue={params.status ?? ""}
          options={[
            { value: "", label: t(locale, "consoleUserAllStatus") },
        ...
        <Select
          ...
          options={[
            { value: "", label: t(locale, "consoleUserAllIdentity") },
        ...
        />
        <Button>{t(locale, "consoleUserSearch")}</Button>
      </form>
      {error ? (
        <Card className="flex items-center justify-between gap-4 border-destructive/20 bg-destructive/5 p-5">
          <div>
            <p className="font-medium text-destructive">{t(locale, "consoleUserLoadFailed")}</p>
      ...
      <Card className="overflow-hidden">
        {users.length ? (
          ...
          <div className="space-y-4 p-8 text-center">
            <p className="text-sm text-muted-foreground">{t(locale, "consoleUserNotFound")}</p>
            <p className="text-xs text-muted-foreground">{t(locale, "consoleUserNotFoundHint")}</p>
          </div>
        )}
      </Card>
    </div>
