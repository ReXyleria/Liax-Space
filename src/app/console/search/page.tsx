...
import { searchConsoleContent } from "@/features/console-search/service";
...
function labels(locale: Awaited<ReturnType<typeof getConsoleLocale>>) {
  return locale === "en"
    ? {
        title: "Search",
        description: "Search console content you are allowed to manage.",
        empty: "No results found.",
        emptyHint: "Try another keyword or open the related console section directly.",
...
export default async function ConsoleSearchPage({
  ...
  const [locale, user] = await Promise.all([
    getConsoleLocale(),
    requireConsoleAccess("/console/search")
  ]);
  const text = labels(locale);
  const query = params.q?.trim() ?? "";
  const { groups, error } = query
    ? await searchConsoleContent(user, query, locale)
    : { groups: [], error: null as string | null };

  return (
    <div className="space-y-6">
      <ConsolePageHeader
        eyebrow={locale === "en" ? "Console" : "后台"}
