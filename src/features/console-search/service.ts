...
function settingHref(key: string) {
  if (key.startsWith("home.") || key.startsWith("theme.") || key.startsWith("appearance.")) {
    return "/console/settings/homepage";
  }
  if (key.startsWith("footer.") || key.startsWith("record.") || key.startsWith("contact.")) {
    return "/console/settings/footer";
  }
  if (key.startsWith("passkey.") || key.includes("requireApproval")) {
    return "/console/settings/security";
  }
  return "/console/settings/basic";
}
...
export async function searchConsoleContent(user: CurrentUser, rawQuery: string, locale: Locale) {
  const query = rawQuery.trim();
  if (!query) {
    return { groups: [] as ConsoleSearchGroup[], error: null as string | null };
  }
  if (!isDatabaseConfigured()) {
    return { groups: [] as ConsoleSearchGroup[], error: "DATABASE_URL is not configured." };
  }
    ...
    if (canManageSettings(user)) {
      ...
      groups.push({
        ...
        results: media.map((asset) => ({
          ...
          href: `/console/data/media?q=${encodeURIComponent(asset.filename)}`,
          meta: asset.mimeType
        }))
      });
    ...
    if (canManageUsers(user)) {
      ...
      groups.push({
        ...
        results: users.map((targetUser) => ({
          ...
          href: `/console/users?q=${encodeURIComponent(targetUser.email || targetUser.username || targetUser.nickname)}`,
