...
function templateLabel(locale: Locale, scene: MailTemplateScene) {
  if (locale === "en") {
    switch (scene) {
      ...
      case MailTemplateScene.TOTP_DISABLE_CODE:
        return { name: "Disable TOTP verification code", description: "Sent when a user disables TOTP by email verification." };
      case MailTemplateScene.TOTP_RECOVERY_USED:
        return { name: "TOTP recovery code used", description: "Sent when a recovery code signs in and disables TOTP." };
