...
function text(locale: Locale) {
  return locale === "en"
    ? {
        ...
        deleteDescription: "The moment will be hidden from the public site and the console list.",
        images: "Images",
        pinnedLabel: "Pinned",
        publicLabel: "Public"
      }
...
function MomentEditDialog({ locale, moment }: { locale: Locale; moment: ConsoleMomentRow }) {
...
export function ConsoleMomentManager({ locale, moments }: { locale: Locale; moments: ConsoleMomentRow[] }) {
  const router = useRouter();
  const copy = text(locale);
