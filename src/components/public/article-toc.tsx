...
export function ArticleToc({ items }: { items: TocItem[] }) {
  ...
  return (
    <>
      <button
        type="button"
        className="fixed right-0 top-1/2 z-[90] grid h-14 w-8 -translate-y-1/2 place-items-center rounded-l-full border border-r-0 border-white/90 bg-white/88 text-muted-foreground shadow-soft transition duration-300 hover:w-10 hover:bg-white hover:text-primary"
      ...
      <div className={cn("fixed inset-0 z-[130]", open ? "pointer-events-auto" : "pointer-events-none")}>
        <button
          type="button"
          className={cn(
