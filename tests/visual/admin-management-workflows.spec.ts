...
test("roles, tags, and attachment library mutate only after deliberate user actions", async ({ page }) => {
  ...
  const roleDialog = page.getByRole("dialog", { name: "Add identity" });
  ...
  await page.getByLabel("Chinese slug").fill("design");
  await page.getByLabel("English name").fill("Design");
  await page.getByLabel("English slug").fill("design-en");
  ...
  await newTag.getByLabel("English slug").fill("design-notes");
  ...
  expect(state.tagUpdateRequests).toEqual([
    { locale: "zh-CN", name: "设计", slug: "design", tagId: 2 },
    { locale: "en-US", name: "Design Notes", slug: "design-notes", tagId: 2 }
  ]);
  ...
  await expect(page.getByRole("button", { name: "Clean selected unused attachments" })).toBeDisabled();
  await page.getByLabel("Select attachment #11").check();
  await page.getByRole("button", { name: "Clean selected unused attachments" }).click();
